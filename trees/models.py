...
class Tree(CommonModel):
    ...
    health = models.CharField(
        max_length=20, choices=HealthChoices.choices, null=True, blank=True
    )
    last_update = models.DateField(null=True, blank=True)
    notes = models.TextField(null=True, blank=True)
    alternate_tag = models.CharField(
        max_length=20, null=True, blank=True, verbose_name="Tag ##"
    )
    quantity = models.IntegerField(default=1)

    # Relationships
    contributors = models.ManyToManyField(
        User, blank=True, related_name="contributed_trees"
    )
