            ...
            for row in reader:
                try:
                    ...
                    if Tree.objects.filter(tag_number=tag_number).exists():
                        self.stdout.write(
                            self.style.WARNING(
                                f"Tree with tag {tag_number} already exists. Skipping."
                            )
                        )
                        continue
                    ...
                    if count % 100 == 0:
                        self.stdout.write(
                            self.style.SUCCESS(f"Imported {count} trees so far...")
                        )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f'Error importing tree {row.get("Tag #", "unknown")}: {str(e)}'
                        )
                    )

        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully imported {count} trees with {errors} errors"
            )
        )
