...
def tree_list(request):
    ...
    # 필터 선택지 가져오기
    common_name_choices = Tree.objects.values_list('common_name', flat=True).exclude(common_name__isnull=True).exclude(common_name__exact='').distinct().order_by('common_name')
    health_choices = Tree.objects.values_list('health', flat=True).distinct().order_by('health')
